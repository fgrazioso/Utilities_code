...
rgb_result4 = cv2.addWeighted(rgb_inverse_result,1,rgb_masked_result3,1,0)
...
low_edge = np.array([160, 60, 60])
up_edge = np.array([190,255,255])
mask2 = cv2.inRange(hsv_image,low_edge,up_edge)

mask3 = mask1 + mask2

# obtain a "masked result" (result1) for the "orange oil only"
rgb_masked_result5 = cv2.bitwise_and(rgb_result4, rgb_result4, mask=mask3)
#plt.imshow(rgb_masked_result1)
#plt.show()

#sys.exit()

# obtain a "darkened result" (result2) changing brightness and contrast
rgb_masked_result6 = cv2.convertScaleAbs(rgb_masked_result5, alpha=0.3, beta=-10)
# this changes the brightness (alpha [1.0-3.0]) and contrast (beta [0-100]) 


# create an inverted mask, for the "intermediate image result minus the "orange oil "
inverse_mask = cv2.bitwise_not(mask3)
rgb_inverse_result = cv2.bitwise_and(rgb_result4, rgb_result4, mask=inverse_mask)


# create a masked version of the darkened result
rgb_masked_result7 = cv2.bitwise_and(rgb_masked_result6, rgb_masked_result6, mask=mask)


# merge the inverted masked original image with the 
rgb_finalresult = cv2.addWeighted(rgb_inverse_result,1,rgb_masked_result7,1,0)


final_blurred = cv2.blur(rgb_finalresult, (3, 3))
